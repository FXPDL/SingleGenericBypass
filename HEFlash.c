...
char HEFLASH_writeBlock (char radd, char* data, char count) {
    ...
    uint16_t add = radd * WRITE_FLASH_BLOCKSIZE + HEFLASH_START;
    // 2. check input parameters
    if ((count > WRITE_FLASH_BLOCKSIZE)||(radd >= HEFLASH_MAXROWS))
...
char HEFLASH_readBlock (char *buffer, char radd, char count) {
    // 1. obtain absolute address in HE FLASH row
    unsigned add = radd * WRITE_FLASH_BLOCKSIZE + HEFLASH_START;
    // 2. check input parameters
    if ((count > WRITE_FLASH_BLOCKSIZE)||(radd >= HEFLASH_MAXROWS))
    return -1;
    // 3. read content
...
char HEFLASH_readByte (char radd, char offset) {
    ...
    unsigned add = radd * WRITE_FLASH_BLOCKSIZE + HEFLASH_START + offset;
    // 2. read content
